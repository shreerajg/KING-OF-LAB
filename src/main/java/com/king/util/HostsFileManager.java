...
public class HostsFileManager {
    ...
    private static final String BACKUP_PATH = "C:\\Windows\\System32\\drivers\\etc\\hosts.king.bak";
    private static final String KING_MARKER_START = "# ===== KING LAB BLOCKER START =====";
    private static final String KING_MARKER_END = "# ===== KING LAB BLOCKER END =====";
    ...
    private static String buildBlockEntries() {
        ...
        sb.append("\n").append(KING_MARKER_START).append("\n");
        sb.append("# Blocked by King Lab Management - DO NOT EDIT\n");
        for (String domain : BLOCKED_DOMAINS) {
            sb.append("127.0.0.1 ").append(domain).append("\n");
        }
        sb.append(KING_MARKER_END).append("\n");
        return sb.toString();
    }
    ...
    public static synchronized boolean blockSites() {
        try {
            ...
            if (currentContent.contains(KING_MARKER_START)) {
    ...
    private static boolean blockSitesElevated() {
        try {
            ...
            Path tempScript = Files.createTempFile("king_block_", ".ps1");
            ...
            if (content.contains(KING_MARKER_START)) {
                blocked = true;
    ...
    /**
     * Restore the original hosts file by removing King's block entries.
     * Automatically elevates to admin if needed.
     */
    public static synchronized boolean restoreHostsFile() {
        try {
            ...
            String content = new String(Files.readAllBytes(Paths.get(HOSTS_PATH)));
            if (!content.contains(KING_MARKER_START) && !Files.exists(Paths.get(BACKUP_PATH))) {
                blocked = false;
    ...
    private static boolean restoreDirect() {
        try {
            ...
            if (Files.exists(backupPath)) {
                ...
                if (content.contains(KING_MARKER_START)) {
                    int startIdx = content.indexOf(KING_MARKER_START);
                    int endIdx = content.indexOf(KING_MARKER_END);
                    if (endIdx > startIdx) {
                        String cleaned = content.substring(0, startIdx)
                                + content.substring(endIdx + KING_MARKER_END.length());
    ...
    private static boolean restoreElevated() {
        try {
            Path tempScript = Files.createTempFile("king_restore_", ".ps1");
            StringBuilder ps = new StringBuilder();
            ...
            ps.append("    $startMarker = '").append(KING_MARKER_START).append("'\n");
            ps.append("    $endMarker = '").append(KING_MARKER_END).append("'\n");
    ...
    /**
     * Check if the hosts file has leftover King entries (e.g. from a crash).
     */
    public static boolean hasLeftoverBlocks() {
        try {
            String content = new String(Files.readAllBytes(Paths.get(HOSTS_PATH)));
            return content.contains(KING_MARKER_START);
        } catch (IOException e) {
            return false;
        }
    }
